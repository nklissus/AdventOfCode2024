topo_map = File.readlines('input.txt').map(&:strip).map { |x| x.split('').map(&:to_i) }
...
def find_trails(map, trailhead)
    ...
    next_spaces.uniq.length
end


tot_score = 0
trailheads.each do |head|
    tot_score += find_trails(topo_map, head)
end

puts "The sum of all trailhead scores is #{tot_score}"